...
// CommitsClient defines Gitlab File service operations
...
// NewFileClient returns a new Gitlab Project service
func NewFileClient(cfg clients.Config) FileClient {
	git := clients.NewClient(cfg)
	return git.RepositoryFiles
}

func NewCommitsClient(cfg clients.Config) CommitClient {
	git := clients.NewClient(cfg)
	return git.Commits
}
...
func LateInitializeFile(in *v1alpha1.FileParameters, file *gitlab.File, commit *gitlab.Commit) { //nolint:gocyclo
	if file == nil {
		return
	}
...
func IsFileUpToDate(p *v1alpha1.FileParameters, g *gitlab.File) bool { //nolint:gocyclo
